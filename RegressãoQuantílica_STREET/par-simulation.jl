...
dem = ts(Y[1:n,1], start = c(1,1), frequency = 1)
sim = ts(tail(Y, max_sim), start = c(n+1,1), frequency = 1)
...
plot(ts(0, start = inicio.serie), xlim=c(start(dem)[1],end(sim)[1]+1), ylim = limites.y, main = title, ylab = "MW")
 lines(ts(c(dem,cenario.medio[1]), start = inicio.serie, frequency = 12), col = 3)

 # dem.ts <- ts(dem, start = inicio.serie, frequency = 12)


 lines(ts(cenario.medio, start = inicio.previsao, frequency = 12), col = 1);
 ...
 lines(ts(lower[1,], start = inicio.previsao, frequency = 12), col = 5, lty = 2);
 lines(ts(upper[1,], start = inicio.previsao, frequency = 12), col = 5, lty = 2);
 lines(ts(lower[2,], start = inicio.previsao, frequency = 12), col = 2);
 lines(ts(upper[2,], start = inicio.previsao, frequency = 12), col = 2);
