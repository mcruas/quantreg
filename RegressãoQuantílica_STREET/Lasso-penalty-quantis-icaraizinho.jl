###########################################################################
#  Este arquivo produz os gráficos disponíveis na pasta 'Documento Regressao Quantilica/Figuras/Lasso-penalty-quantis-icaraizinho'
...
nome_pasta = "NORMALIZED"
try mkdir("Documento Regressao Quantilica/Figuras/Lasso-penalty-quantis-icaraizinho/$nome_pasta/") end
for lambda = [0.1, 0.3, 1.0,3.0,10.0,20.0,30.0,50.0,100.0]   # lambda = 2.
    for gamma = [0.1, 0.3, 1.0, 3.0, 10.0,20.0]
        ...
        results_lasso = rq_par_lasso_normalize(y, X, Alphas; lambda = lambda, gamma = 0, non_cross = true) # estimates the normal lasso
        p = plot(Alphas, results_lasso[:2]', legend = false, xlab = "\$\\alpha\$", ylab = "\$\\beta_{p} (\\alpha)\$", title = "(I)\$\\lambda=$lambda \\quad \\gamma_1=0 \\quad n=$n\$")
        results_penlasso = rq_par_lasso_normalize(y, X, Alphas; lambda = lambda, gamma = gamma, non_cross = true) # estimates lasso with penalization of derivatives
        ...
        # results_adap1 = rq_par_lasso_normalize(y, X, Alphas; lambda = lambda, w = w1, gamma = gamma, non_cross = true)
        results_adap_lasso = rq_par_lasso_normalize(y, X, Alphas; lambda = lambda, w = w_lasso,  non_cross = true)
        r = plot(Alphas, results_adap_lasso[:2]', legend = false, xlab = "\$\\alpha\$", ylab = "\$\\beta_{p} (\\alpha)\$", title = "Adaptive w=(I) \$\\gamma_2 = 0\$")
        
        results_adap_penlasso = rq_par_lasso_normalize(y, X, Alphas; lambda = lambda, w = w_penlasso, gamma = gamma, non_cross = true)
        s = plot(Alphas, results_adap_penlasso[:2]', legend = false, xlab = "\$\\alpha\$", ylab = "\$\\beta_{p} (\\alpha)\$", title = "Adaptive w=(II) \$\\gamma_2 = $gamma\$")
        
        results_adap_penlasso_notpen = rq_par_lasso_normalize(y, X, Alphas; lambda = lambda, w = w_penlasso, gamma = 0, non_cross = true)
        t = plot(Alphas, results_adap_penlasso_notpen[:2]', legend = false, xlab = "\$\\alpha\$", ylab = "\$\\beta_{p} (\\alpha)\$", title = "Adaptive w=(II) \$\\gamma_2 = 0\$")
        
        results_adap_lasso_wn = rq_par_lasso_normalize(y, X, Alphas; lambda = lambda, w = w_lasso_wn,  non_cross = true)
        u = plot(Alphas, results_adap_lasso_wn[:2]', legend = false, xlab = "\$\\alpha\$", ylab = "\$\\beta_{p} (\\alpha)\$", title = "Adaptive wn=(I) \$\\gamma_2 = 0\$ ")
        
        results_adap_penlasso_wn = rq_par_lasso_normalize(y, X, Alphas; lambda = lambda, w = w_penlasso_wn, gamma = gamma, non_cross = true)
        v = plot(Alphas, results_adap_penlasso_wn[:2]', legend = false, xlab = "\$\\alpha\$", ylab = "\$\\beta_{p} (\\alpha)\$", title = "Adaptive wn=(II) \$\\gamma_2 = $gamma\$")
        
        results_adap_penlasso_wn_notpen = rq_par_lasso_normalize(y, X, Alphas; lambda = lambda, w = w_penlasso_wn, gamma = 0, non_cross = true)
        ...
        plot(p,r,u, q ,s,t,v,v1,size = (1200,700), ylim = (-1.0,1.0))

        name_file = replace("Lambda$lambda-gamma$gamma",".","")
        savefig("Documento Regressao Quantilica/Figuras/Lasso-penalty-quantis-icaraizinho/$nome_pasta/$name_file.pdf")
        savefig("Documento Regressao Quantilica/Figuras/Lasso-penalty-quantis-icaraizinho/$nome_pasta/$name_file.png")
