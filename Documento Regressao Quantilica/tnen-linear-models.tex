...
\begin{equation}
\min_{q_t}\sum_{t \in T_\tau}^{n}\alpha|y_{t}-q(x_t)|^{+}+(1-\alpha)|y_{t}-q(x_t)|^{-},
\end{equation}
...
\mbox{s.t. } & \varepsilon_{t,\alpha}^{+}-\varepsilon_{t,\alpha}^{-}=y_{t}- \beta_{0}^\alpha - (\beta^{\alpha})^T x_{t}, & \qquad\forall t \in T_\tau,\forall \alpha \in A,\\
