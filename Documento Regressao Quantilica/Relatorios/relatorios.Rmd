...
## Wind Power Time Series - Icaraizinho
...
- For a single quantile, the problem  (\ref{eq:optim-lqr1}) can be solved by the following Linear Programming problem:
$$
\begin{array}{lll}
...
\end{array}
$$
- The output are the coefficients $\beta_0$ and $\beta$ (which is the same dimension as $x_t$), that describe describes the quantile function as an affine function.
...
- Mixed Integer Linear Programming (MILP) models allow only $K$ variables to be used for each $\alpha$-quantile. This means that only $K$ coefficients $\beta_{pj}$ may have nonzero values, for each $\alpha$-quantile. It must be guaranteed by constraints on the optimization problem.

- We present three forms of regularization using MILP


## MILP - One model for each $\alpha$-quantile
...
<!-- \tiny -->
<!-- \begin{eqnarray} -->
<!--  \underset{\beta_{0j},\beta_j,z_{p j} \varepsilon_{t j}^{+},\varepsilon_{t j}^{-}}{\text{min}} & \sum_{j \in J} \sum_{t\in T}\left(\alpha\varepsilon_{t j}^{+}+(1-\alpha)\varepsilon_{t\alpha}^{-}\right) \label{eq:mip0} \\ -->
<!-- \mbox{s.t } & \varepsilon_{t j}^{+}-\varepsilon_{t j}^{-}=y_{t}-\beta_{0 j}-\sum_{p=1}^{P}\beta_{p j}x_{t,p},& \qquad\forall t \in T ,\forall j \in J, \label{eq:mip1}\\ -->
<!-- & \varepsilon_{t j}^{+},\varepsilon_{t j}^{-}\geq0,&\qquad\forall t \in T ,\forall j \in J, \label{eq:mip2}\\ -->
<!-- & - M z_{p \alpha g} \leq \beta_{p j} \leq M z_{p \alpha g},&\qquad \forall j \in J, \forall p\in P, \forall g \in G \label{eq:mip3}\\ -->
<!-- &z_{p \alpha g} := 2 - ( 1-z_{pg}) - I_{g\alpha}& \label{mipgrupzpa} \\ -->
<!-- & \sum_{p=1}^P z_{p g} \leq K, & \qquad \forall g \in G, \label{eq:mip4}\\ -->
<!-- & \beta_{0j} + \beta_{\alpha}^T x_{t} \leq \beta_{0\alpha'} + \beta_{\alpha'}^T x_{t}, & \qquad \forall t \in T, \forall (\alpha, \alpha') \in A \times A,  \alpha < \alpha',\nonumber\\ \label{eq:mip6} \\ -->
...
<!-- \tiny -->
<!-- \begin{eqnarray} -->
<!-- \underset{\beta_{0j},\beta_j,z_{p j} \varepsilon_{t \alpha}^{+},\varepsilon_{t \alpha}^{-}}{\text{min}} & \sum_{\alpha \in A} \sum_{t\in T}\left(\alpha\varepsilon_{t \alpha}^{+}+(1-\alpha)\varepsilon_{t\alpha}^{-}\right) \label{eq:mipgr0} \\ -->
<!-- \mbox{s.t } & \varepsilon_{t \alpha}^{+}-\varepsilon_{t \alpha}^{-}=y_{t}-\beta_{0 j}-\sum_{p=1}^{P}\beta_{p j}x_{t,p},& \qquad\forall t \in T ,\forall \alpha \in A, \label{eq:mipgr1}\\ -->
<!-- & \varepsilon_{t \alpha}^{+},\varepsilon_{t \alpha}^{-}\geq0,&\qquad\forall t \in T ,\forall \alpha \in A, \label{eq:mipgr2}\\ -->
<!-- & - M z_{p j} \leq \beta_{p j} \leq M z_{p j},&\qquad \forall \alpha \in A, \forall p\in\{1,\dots,P\}, \label{eq:mipgr3}\\ -->
<!-- & \sum_{p=1}^P z_{p j} \leq K, & \qquad \forall \alpha \in A, \label{eq:mipgr4}\\ -->
<!-- & z_{p j} \in \{0,1\},&\qquad \forall \alpha \in A, \forall p\in\{1,\dots,P\}, \label{eq:mipgr5}\\ -->
...
- Another way of doing regularization is including the coefficients $\ell_1$-norm on the objective function
