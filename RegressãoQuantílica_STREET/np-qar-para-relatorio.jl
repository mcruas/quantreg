# include("np-qar-para-relatorio.jl")
...
for ilamb in 1:length(lambdas) # Itera so bos valores de lambda
	...
	plot(x,y, "r." , xlabel = "y_{t-1}", ylabel = "y_{t}")
	oplot(x[I],thetasoptMat[I,1], "b-") # 0.05
	oplot(x[I],thetasoptMat[I,2], "g-") # 0.1
	oplot(x[I],thetasoptMat[I,3], "y-") # 0.25
	oplot(x[I],thetasoptMat[I,4], "-") # 0.5
